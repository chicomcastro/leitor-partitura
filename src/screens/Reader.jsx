      ...
      {chromeVisible && (
        <div className={s.topBar}>
          <div className={s.topRow}>
            <button className={s.backBtn} onClick={handleBack} aria-label="Voltar para a estante">
            ...
            <div className={s.titleWrap}>
              ...
              <div className={s.pageLabel} aria-live="polite">Página {currentPage} de {totalPages}</div>
            </div>
            {hasPieceNav && (
              <div className={s.pieceNav}>
                <button className={s.chromeBtn} onClick={prevPiece} title="Peça anterior" aria-label="Peça anterior">
                ...
                <button className={s.chromeBtn} onClick={nextPiece} title="Próxima peça" aria-label="Próxima peça">
            ...
            <button className={s.fitBtn} onClick={() => setFitMode(fitMode === 'page' ? 'width' : 'page')} aria-label={`Modo de ajuste: ${fitMode === 'page' ? 'Página' : 'Largura'}`}>
            ...
            <button className={s.fitBtn} onClick={() => setPageMode(m => m === 'full' ? 'half' : 'full')} aria-label={`Modo de rolagem: ${pageMode === 'full' ? 'Página inteira' : 'Meia página'}`}>
          ...
          <div className={s.markersRow}>
            ...
            <div className={s.markersScroll}>
              {markers.map(m => (
                <div key={m.id} className={s.marker}>
                  <button className={s.markerGoBtn} onClick={() => gotoPage(m.page, true)} aria-label={`Ir para ${m.label}, página ${m.page}`}>{m.label} · p.{m.page}</button>
                  <button className={s.markerDelBtn} onClick={() => setMarkers(markers.filter(x => x.id !== m.id))} aria-label={`Remover marcador ${m.label}`}>
            ...
            <button className={s.addMarkerBtn} onClick={() => setModal({ type: 'marker' })} aria-label="Criar marcador de salto">
      ...
      {chromeVisible && annotating && (
        <div className={s.annotBar}>
          {['#E73B4C', '#3B82F6', '#1a1a1a'].map(c => {
            const colorName = c === '#E73B4C' ? 'Vermelho' : c === '#3B82F6' ? 'Azul' : 'Preto'
            return (
              <button
                key={c}
      ...
      {chromeVisible && (
        <div className={s.bottomBar}>
          <div className={s.toolbar}>
            <button className={autoscroll ? s.toolBtnActive : s.toolBtnInactive} onClick={toggleAutoscroll} aria-label={autoscroll ? 'Parar rolagem automática' : 'Iniciar rolagem automática'} aria-pressed={autoscroll}>
            ...
            <div className={s.speedGroup}>
              ...
              <input type="range" min="8" max="280" step="1" value={scrollSpeed} onChange={e => setScrollSpeed(+e.target.value)} style={{ flex: 1 }} aria-label={`Velocidade de rolagem: ${scrollSpeed} pixels por segundo`} />
              <span className={s.speedLabel}>{scrollSpeed} px/s</span>
            </div>
            <div className={s.divider} />
            <button className={metro.running ? s.toolBtnActive : s.toolBtnGhost} onClick={() => setShowMetro(true)} title="Metrônomo" aria-label="Metrônomo">
            ...
            <button className={recorder.recording ? s.toolBtnRec : s.toolBtnGhost} onClick={recorder.toggleRec} title="Gravar" aria-label={recorder.recording ? 'Parar gravação' : 'Gravar'}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            ...
            <button className={s.toolBtnGhost} onClick={() => setShowRecordings(true)} title="Gravações" aria-label="Gravações">
            ...
            <button className={annotating ? s.toolBtnActive : s.toolBtnGhost} onClick={() => setAnnotating(a => !a)} title="Anotar" aria-label="Anotar" aria-pressed={annotating}>
            ...
            <button className={s.toolBtnGhost} onClick={() => setShowGestures(true)} title="Configurar gestos" aria-label="Configurar gestos">
