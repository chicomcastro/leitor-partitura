...
export default function Reader({
  ...
  return (
    <div className={s.root}>
      <div ref={viewerRef} className={s.viewer} />
        ...
        <div className={s.topBar}>
          <div className={s.topRow}>
            <button className={s.backBtn} onClick={handleBack}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M20,11V13H8L13.5,18.5L12.08,19.92L4.16,12L12.08,4.08L13.5,5.5L8,11H20Z" /></svg>
            </button>
            <div className={s.titleWrap}>
              <div className={s.title}>{scoreName}</div>
              <div className={s.pageLabel}>Página {currentPage} de {totalPages}</div>
            </div>
            {hasPieceNav && (
              <div className={s.pieceNav}>
                <button className={s.chromeBtn} onClick={prevPiece} title="Peça anterior">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6,18V6H8V18H6M9.5,12L18,6V18L9.5,12Z" /></svg>
                </button>
                <button className={s.chromeBtn} onClick={nextPiece} title="Próxima peça">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M16,18H18V6H16M6,18L14.5,12L6,6V18Z" /></svg>
                </button>
              </div>
            )}
          ...
          <div className={s.markersRow}>
            ...
            <div className={s.markersScroll}>
              {markers.map(m => (
                <div key={m.id} className={s.marker}>
                  <button className={s.markerGoBtn} onClick={() => gotoPage(m.page, true)}>{m.label} · p.{m.page}</button>
                  <button className={s.markerDelBtn} onClick={() => setMarkers(markers.filter(x => x.id !== m.id))}>
            ...
            <button className={s.addMarkerBtn} onClick={() => setModal({ type: 'marker' })}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M17,18L12,15.82L7,18V5H17M17,3H7A2,2 0 0,0 5,5V21L12,18L19,21V5C19,3.89 18.1,3 17,3Z" /></svg>
              Salto
            </button>
          </div>
        </div>
      ...
      {chromeVisible && (
        <div className={s.bottomBar}>
          <div className={s.toolbar}>
            <button className={autoscroll ? s.toolBtnActive : s.toolBtnInactive} onClick={toggleAutoscroll}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
                <path d={autoscroll ? 'M14,19H18V5H14M6,19H10V5H6V19Z' : 'M8,5.14V19.14L19,12.14L8,5.14Z'} />
              </svg>
            </button>
            <div className={s.speedGroup}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="var(--text-muted)" style={{ flex: 'none' }}><path d="M7.41,8.58L12,13.17L16.59,8.58L18,10L12,16L6,10L7.41,8.58Z" /></svg>
              <input type="range" min="8" max="280" step="1" value={scrollSpeed} onChange={e => setScrollSpeed(+e.target.value)} style={{ flex: 1 }} />
              <span className={s.speedLabel}>{scrollSpeed} px/s</span>
            </div>
            <div className={s.divider} />
            <button className={metro.running ? s.toolBtnActive : s.toolBtnGhost} onClick={() => setShowMetro(true)} title="Metrônomo">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M12,1.75L8.57,2.67L4.07,19.5C3.9,20.15 4.05,20.84 4.46,21.37C4.87,21.91 5.5,22.21 6.17,22.21H17.83C18.5,22.21 19.13,21.91 19.54,21.37C19.95,20.84 20.1,20.15 19.93,19.5L15.43,2.67L12,1.75M10.29,4.6L11.27,4.07H12.73L13.71,4.6L17.65,19.4C17.71,19.62 17.65,19.79 17.5,19.97C17.35,20.15 17.16,20.21 16.95,20.21H7.05C6.84,20.21 6.65,20.15 6.5,19.97C6.35,19.79 6.29,19.62 6.35,19.4L10.29,4.6Z" /></svg>
            </button>
            <button className={recorder.recording ? s.toolBtnRec : s.toolBtnGhost} onClick={recorder.toggleRec} title="Gravar">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
