    ...
    <div className={s.panelBackdrop} onClick={onClose}>
      <div className={s.panel} onClick={e => e.stopPropagation()}>
        <div className={s.panelHeader}>
          <div className={s.panelTitle}>{t('metro.title')}</div>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600, letterSpacing: 1, marginTop: 2 }}>{t('metro.bpm')}</div>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, margin: '18px 0' }} aria-label={`${t('metro.beatIndicator')}: ${beats} ${t('metro.beat').toLowerCase()}`} role="group">
          {metroBeats.map((b, i) => (
            <div key={i} style={{ width: b.sz, height: b.sz, borderRadius: '50%', background: b.col, transition: 'all .08s' }} aria-label={`${t('metro.beat')} ${i + 1}${running && i === currentBeat ? ', ' + t('metro.beatActive') : ''}`} />
          ))}
        </div>

        <input type="range" min="40" max="240" step="1" value={bpm} onChange={e => setBpm(+e.target.value)} style={{ width: '100%', marginBottom: 18 }} aria-label={`${t('metro.bpm')}: ${bpm}`} />

        <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
          <button onClick={tapTempo} aria-label={t('metro.tapLabel')} style={{ flex: 1, background: 'var(--surface-hover)', border: '1px solid var(--border-light)', color: 'var(--text)', padding: 11, borderRadius: 'var(--radius-md)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            {t('metro.tap')}
          </button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
          <span style={{ fontSize: 13, color: 'var(--text-secondary)', fontWeight: 600 }}>{t('metro.timeSignature')}</span>
