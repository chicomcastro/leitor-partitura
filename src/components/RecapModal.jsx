...
import { drawShareCard, shareOrDownloadCard } from '../lib/shareCard'
import { formatDuration } from '../lib/stats'
import s from './MilestoneModal.module.css'

// Weekly recap shown once at the start of a new week, summarizing the previous
// week with a shareable card. See ADR 022.
export default function RecapModal({ summary, onClose }) {
  const { t } = useI18n()
  const cardRef = useRef(null)

  const handleShare = async () => {
    const cardSummary = {
      totalLabel: formatDuration(summary.ms),
      badge: t('recap.badge'),
      subtitle: `${summary.activeDays} ${t('recap.activeDays')}`,
      topPiece: summary.topPiece,
      topComposer: null,
    }
    const canvas = cardRef.current || document.createElement('canvas')
    drawShareCard(canvas, cardSummary, {
      heroLabel: t('recap.heroLabel'),
      topPieceLabel: t('stats.topPiece'),
      topComposerLabel: t('stats.topComposer'),
    })
    await shareOrDownloadCard(canvas, 'tapscore-semana.png')
  }
  ...
  return (
    <div className={s.backdrop} onClick={onClose}>
      <div className={s.panel} onClick={e => e.stopPropagation()} role="dialog" aria-modal="true">
        ...
        <div className={s.label}>{t('recap.title')}</div>
        <div className={s.headline}>{formatDuration(summary.ms)}</div>
        <div className={s.sub}>
          {summary.activeDays} {t('recap.activeDays')}
          {summary.topPiece ? ` · ${summary.topPiece}` : ''}
        </div>
